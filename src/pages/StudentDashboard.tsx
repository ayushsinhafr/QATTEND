...
const StudentDashboard = () => {
  ...
  // Check face enrollment status and preload models on component load
  useEffect(() => {
    ...
    // Preload face recognition models for students only
    const preloadFaceModels = async () => {
      if (user?.id && profile?.role === 'student') {
        console.log('🎯 [StudentDashboard] Preloading face recognition models for student...');
        try {
          await FaceModelManager.preloadModel();
          console.log('✅ [StudentDashboard] Face models preloaded successfully');
        } catch (error) {
          console.error('❌ [StudentDashboard] Failed to preload face models:', error);
        }
      }
    };
        ...
        return new Promise((resolve) => {
          requireFaceVerification(
            ...
            async () => {
              ...
              // Close the scanner dialog after successful face verification
  ...
  const stopScanner = useCallback(() => {
    ...
    setIsProcessingQR(false);
    setLastScannedQR("");
    setShowScanner(false); // Ensure dialog is closed
  }, []);
  ...
  */
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      ...
      {/* Standard Face Verification Modal */}
